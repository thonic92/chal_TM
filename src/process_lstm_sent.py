...
@click.argument('save_dir', type=click.Path(), default = 'models/lstm_sent/version1')
@click.argument('model_dir', type=click.Path(), default = 'models/lstm_sent/version1')
@click.argument('nb_token_keep', type=int, default = 6676)
...
def main(goal, tokens_dir, save_dir, model_dir, nb_token_keep, lstm_hidden_size, batch, epochs, step_per_epoch, model_name):
	...
	if goal == 'run':
		...
		# trainer = ModelTrainer(model = lstm_model, data_loader = sentence_loader, epochs =  epochs)
